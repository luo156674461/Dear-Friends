    ...
    methods: {
        ...
        alter(name) {
            ...
            this.$refs[name].validate((valid) => {
                if (valid) {
                    let params = {}
                    params.new_passwd = this.$des.encrypt(this.formInline.pass)
                    params.cell = this.$des.decrypt(this.info.cell)
                    params.random_code = this.formInline.cell_ver
                    let token = this.$des.encrypt(`${this.mess.token}${this.arrNum}`)
                    this.$api.post(`rs_login_passwd/?user=${this.mess.user}&token=${token}`, params, r => {
                        let _this = this
                        if (r.state === 200) {
                            this.$Modal.warning({
                                title: r.msg,
